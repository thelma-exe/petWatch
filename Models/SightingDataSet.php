...
class SightingDataSet {
    ...
    // Fetches all sightings with pet name and species for the map
    public function fetchAllSightings() {
        // SQL query to join sightings table with pets table to get pet name and species
        $sqlQuery = 'SELECT sightings.*, pets.name as pet_name, pets.species as pet_species
        FROM sightings
        ...
        // Loop through results, create SightingData objects and enrich with pet data
        while ($row = $statement->fetch()) {
            $sightingData = new SightingData($row);
            $sightingData->setPetName($row['pet_name']); // Enrich with pet name from JOIN
            $sightingData->setPetSpecies($row['pet_species']); // Enrich with pet species from JOIN
            $dataSet[] = $sightingData;
        }
        return $dataSet;
    }
}