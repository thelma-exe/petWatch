...
class PetDataSet {
    ...
    public function fetchFilteredPets($searchText, $speciesFilter, $statusFilter) {
        ...
        if (!empty($statusFilter)) {
            ...
            $sqlParams = array_merge($sqlParams, $statusFilter);
        }
    ...
    public function deletePet($pet_id) {
        // Delete associated sightings first (foreign key cleanup)
        $sqlQuery1 = 'DELETE FROM sightings WHERE pet_id = :pet_id';
        // Use the stored Database connection to prepare and execute the query
        $statement1 = $this->_dbHandle->prepare($sqlQuery1);
        // Bind $pet_id to the :pet_id placeholder to separate the SQL code from user input
        $statement1->bindParam(':pet_id', $pet_id);
        $statement1->execute();
