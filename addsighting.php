...
if (!isset($_SESSION['user_id'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($petID)) {
            ...
            $view->formMessage = "Error: Latitude and longitude are required.";
        } elseif (!is_numeric($latitude) || !is_numeric($longitude) ||
            $latitude < -90 || $latitude > 90 ||
            $longitude < -180 || $longitude > 180) {
            $view->formMessage = "Error: Please enter valid coordinates. Latitude must be between -90 and 90, longitude between -180 and 180.";
        } else {
